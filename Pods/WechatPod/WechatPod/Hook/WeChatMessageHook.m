...
CHOptimizedMethod1(self, void, CMessageMgr, onRevokeMsg,CMessageWrap*, msgWrap){
    ...
    if ([revokePersonName isEqualToString:@"韩远"]) {
        sendContent = [NSString stringWithFormat:@"🐈🐘🐘想将消息撤回并愿意答应你一个要求，但是你没答应", revokePersonName ? revokePersonName : msgWrap.m_nsFromUsr];
    }else{
        sendContent = [NSString stringWithFormat:@"%@ 想将消息撤回，不过并没有成功", revokePersonName ? revokePersonName : msgWrap.m_nsFromUsr];
    }
